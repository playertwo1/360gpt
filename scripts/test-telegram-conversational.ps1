...
Assert-True ($runtime -match 'attempt_count = 0' -and $wf101 -match 'attempt_count = attempt_count \+ 1') 'Reabertura reinicia orçamento de tentativas e aceita falha final'
Assert-True ($messages -match 'TELEGRAM_SAFE_LIMIT = 3600' -and $wf102 -match 'part_index') 'Parecer usa partes seguras e idempotentes'
...
Assert-True ($clarificationAi -match 'isContextRequest') 'Reclamação ou dúvida de formato não vira valor de indicador'
