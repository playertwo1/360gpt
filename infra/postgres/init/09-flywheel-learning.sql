...
CREATE INDEX IF NOT EXISTS idx_golden_exemplars_lookup ON golden_exemplars (tenant_id, sector, objective, channel, status);
...
CREATE INDEX IF NOT EXISTS idx_decision_outcomes_domain ON decision_outcomes (tenant_id, domain, outcome_type);
...
CREATE INDEX IF NOT EXISTS idx_negative_memory_entity ON negative_memory (tenant_id, target_entity, vetoed_topic, status);

-- 5. Trilha de Auditoria Append-Only do Flywheel (flywheel_audit_events)
CREATE TABLE IF NOT EXISTS flywheel_audit_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id VARCHAR(50) NOT NULL DEFAULT 'default',
...
CREATE INDEX IF NOT EXISTS idx_flywheel_audit_events_entity ON flywheel_audit_events (tenant_id, entity_type, entity_id);

-- 6. Concessão de Privilégios Mínimos para visao360_app
