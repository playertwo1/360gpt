...
const tasks=[['Importar dados POBJ','Envie um arquivo para criar prioridades reais','Aguardando'],['Revisar pendências','Consulte dúvidas e revisões do Estado 360','Abrir'],['Verificar fontes','Nenhuma empresa é sugerida sem carteira conectada','Seguro']];
...
<button onClick={() => setSheet('gate-p8')} className="flex w-full items-center justify-between rounded-[28px] border border-[#ffbb00]/30 bg-gradient-to-r from-[#2c220d] to-[#1c180f] p-5 text-left">
  ...
  {tab==='carteira'&&<div className="px-6"><section className="rounded-[28px] border border-[#ffd983]/25 bg-[#2b2413] p-5"><b className="text-lg">Carteira não conectada</b><p className="mt-2 text-sm leading-6 text-[#d8c99f]">Nenhuma empresa será inventada. Conecte uma fonte de carteira autorizada para habilitar clientes, contatos e oportunidades.</p></section></div>}
...
<a href="/knowledge" className="flex w-full items-center justify-between rounded-[28px] border border-[#8fb1ff]/25 bg-[#182334] p-5"><span><b>Conhecimento POBJ</b><small className="mt-1 block text-[#b8c9e8]">Mapeamentos homologados, versões e revogações</small></span><small className="text-[#8fb1ff]">ABRIR ›</small></a><a href="/canary" className="flex w-full items-center justify-between rounded-[28px] border border-[#55eca0]/25 bg-[#16281f] p-5"><span><b>Revisar Canary Performance</b><small className="mt-1 block text-[#afd7be]">10 casos sintéticos · aprovar A1</small></span><small className="text-[#55eca0]">ABRIR ›</small></a><a href="/shadow" className="flex w-full items-center justify-between rounded-[28px] bg-[#211d2b] p-5"><span><b>Métricas Shadow</b><small className="mt-1 block text-[#b9b0ca]">Janela, alertas e parecer do gate</small></span><small className="text-[#bba7ff]">ABRIR ›</small></a></div>}
...
function Score(){
 ...
 const current=latest?.approved?.currentPoints??0; const target=latest?.approved?.targetPoints??0; const percent=target?Math.max(0,Math.min(100,Math.round(current/target*100))):0; const remaining=target?Math.max(0,target-current):0;
 return <section className="rounded-[32px] bg-gradient-to-br from-[#242427] to-[#171918] p-6"><div className="flex justify-between text-xs font-bold uppercase tracking-wider text-[#c7cad5]"><span>POBJ atual</span><span className={latest?'text-[#55eca0]':'text-[#ffcc3f]'}>{latest?'dados aprovados':'sem dados reais'}</span></div>{latest?<><div className="mt-5"><b className="text-5xl">{current}</b><span className="ml-2 text-lg text-[#c2c6d6]">/ {target}</span></div><div className="mt-7 h-3 rounded-full bg-[#343434]"><div className="h-full rounded-full bg-[#568dff]" style={{width:`${percent}%`}}/></div><div className="mt-4 flex justify-between text-xs text-[#b9bbc5]"><span>{percent}% atingido</span><span>{remaining} pts para 100%</span></div><p className="mt-3 text-xs text-[#8fb1ff]">Competência {latest.competence} · base {latest.baseDate}</p></>:<p className="mt-5 text-sm leading-6 text-[#c2c6d6]">Envie e aprove um relatório POBJ para exibir pontuação real.</p>}</section>
}
