...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (row.status === 'processed') return NextResponse.json({ ok: false, error: 'already_processed' }, { status: 409 });
  ...
  await env.DB.batch([
    env.DB.prepare(`UPDATE documents SET raw_text = ?, status = 'local_reviewed' WHERE id = ? AND owner_id = ?`).bind(JSON.stringify({ ...meta, official: false, localReview: approved }), id, access.userId),
    env.DB.prepare(`INSERT INTO audit_log (id, owner_id, actor, action, entity_type, entity_id, details_json, created_at) VALUES (?, ?, ?, 'pobj_local_preview_reviewed', 'document', ?, ?, ?)`).bind(`audit-${crypto.randomUUID()}`, access.userId, `chatgpt:${access.email}`, id, JSON.stringify({ ...approved, official:false, awaitingN8n:true }), Date.now()),
  ]);
  return NextResponse.json({ ok: true, id, status: 'local_reviewed', official: false, awaitingN8n: true, localReview: approved });
}
