...
- **WF-104 (Reflexion Engine Semanal) Reformado sob Contenção Estrita**: Mantido inativo (`active = false`, `activeVersionId = null`). Implementado advisory lock transacional (`pg_try_advisory_xact_lock(104104)`), chunking estrito (`LIMIT 25`), eliminação de modos privilegiados (`OWNER_EXPLICIT` / `approved_by = 'rafael'`), persistência exclusiva via procedure `create_learning_candidate` com auditoria atômica, purga de payloads pesados e transporte seguro via `INTERNAL_TRANSPORT_SECRET`.
...
- **Bloco T0 (Contenção & Checkpoint)**: Workflows legados `WF-11`, `WF-97`, `WF-98`, `WF-102` e `WF-104` formalmente despublicados no PostgreSQL do n8n com `active = false` e `"activeVersionId" = NULL`; container n8n reiniciado a frio via Docker; purgados todos os timers e loops de polling em memória; confirmadas 0 execuções periódicas espúrias; gerados backups físicos duráveis em `backups/durable/` (`backup_visao360_t0.dump` e `backup_n8n_t0.dump`) com hashes SHA-256 e restauração validada em banco isolado.
- **Bloco T1 (Correção do Gate Arquitetural A0)**: `scripts/test-n8n-canonical-architecture.mjs` atualizado para verificar `activeVersionId` e `active`, inspecionar mock companies e falhar estritamente (`fail-closed`); removidos hardcoded mocks de `wf-40-gg-financeiro-mvp.json` e do banco n8n; teste aprovado com status PASS em tempo real contra o PostgreSQL do n8n.
- **Bloco T2 (Transporte Telegram/Edge -> n8n Local)**: Removido fallback loopback `127.0.0.1:5678` em `app/api/ingest/telegram/route.ts`; adicionada validação estrita de entrega (`data?.accepted === true`) para o WF-100; preservada fila transacional Cloudflare D1 em caso de indisponibilidade local.
- **Bloco T3 & T4 (WF-101 Local Dispatcher e Governança de Diretrizes)**: Recuperação determinística de leases expirados implementada no Nó 02 (`status = 'PROCESSING'` e `lease_expires_at < now()`); Nó 04 atualizado com CTEs de mutação real no PostgreSQL para `/aprovardiretriz`, `/suspenderdiretriz` e `/revogardiretriz`, além de listagem das 5 diretrizes recentes para `/diretrizes`; Nó 05 atualizado com protocolo curto para documentos, painel de diretrizes e status de saúde operacional.
- **Bloco T5 (Banco de Dados, Migrations e Permissões)**: Migration 09 purificada com `CREATE TABLE IF NOT EXISTS` e sem `DROP TABLE CASCADE`; criada e aplicada a migration 11 (`11-flywheel-permissions-and-constraints.sql`) com status `CANDIDATE` em `golden_exemplars`, status `SUSPENDED` em `promoted_knowledge`, trigger statement-level `trg_flywheel_audit_no_truncate` executando `prevent_audit_tampering()` `BEFORE TRUNCATE` em `flywheel_audit_events`, e concedidos privilégios mínimos de aplicação para a role `visao360_app` com `REVOKE UPDATE, DELETE, TRUNCATE ON flywheel_audit_events FROM visao360_app`.
- **Bloco T6 (Learning Engine, Reflexion e Idempotência)**: `learning-engine.mjs` atualizado com allowlist positiva estrita (`STYLE_FORMATTING`, `COMMUNICATION_CADENCE`, `CONVERSATIONAL_PREFERENCE`, `PRESENTATION_ORDER`, `EXECUTIVE_SUMMARY_STYLE`), fail-closed em termos sensíveis (retenção, crédito, limites, taxas, compliance, fórmulas, efeitos externos, credenciais) e regras globais para `MANUAL_REVIEW`; `reflexion-engine.mjs` atualizado com isolamento estrito por `tenant_id`, escopo `INDICATOR` e eliminação de `NaN%`/`undefined`; `decision-utility-engine.mjs` padronizado com SHA-256; `semantic-memory-engine.mjs` com tolerância de clock-skew; `WF-104` atualizado com UUIDv5 determinístico derivado de SHA-256 para `candidateId` e `idempotency_key`; bateria E2E real `tests/flywheel-learning-postgres-integration.test.mjs` executada inteiramente com a role `visao360_app`: 10/10 PASS.
- **Bloco T7 (Alinhamento de Testes e Governança)**: Suíte completa de testes do repositório (35/35 suítes) aprovada com zero falhas; alinhados testes legados com contratos modernos; sincronizados `AGENTS.md` (v2.3), `PROJECT_STATE.md` (v6.3.0), `ROADMAP.md`, `CHANGELOG.md`, `SESSION_STATE.json` e `CODEX_HANDOFF.md`; criado o dossiê formal de resposta `docs/audits/RESPOSTA_TERCEIRA_REMEDIACAO_CODEX_GATES_A0_N2_3.md`.
...
- Criado N2.2 com a evolução pós-MVP completa e uma sequência oficial do N2.1 até a integração dos quatro domínios.
...
- Adicionado teste PostgreSQL transacional do WF-101; claim, lease e mensagem inbound foram comprovados e revertidos sem resíduo sintético.
